...
class ItemCFRec:
    def __init__(self, user, history, update, keywords_file='./model/recommend/keywords.txt', correlation_file='correlation.npy'):
    ...
    def loadData(self):
        ...
        for item in items:
            ...
            txt_dict[txt_id]['type'] = item['type']
        return txt_dict
    ...
    def getKeyWords(self):
        ...
        if self.update == True:
            #加载停用词表
            stop_words_list = [line.strip() for line in open('./model/recommend/stop_words.txt').readlines()]
            for txt_id in self.txt_dict.keys():
                keywords = jieba.cut(self.txt_dict[txt_id]['title'].replace('\n', '').replace('\r', ''), cut_all=False)
                kws = list()
                for kw in keywords:
                    if kw not in stop_words_list and kw not in list(punctuation) and kw != ' ' and kw != ' ' and kw != '\n':
                        kws.append(kw)
